...
SELECT NUMPIEZA, FECHARECUENTO FROM INVENTARIO WHERE FECHARECUENTO >= TO_DATE('01/10/92','DD/MM/YY') AND FECHARECUENTO <= TO_DATE('31/10/92','DD/MM/YY');
...
SELECT NOMPIEZA, PRECIOVENT FROM PIEZA WHERE NOMPIEZA LIKE '%EGATINA%' AND PRECIOVENT >= 50;
...
SELECT NOMVEND, NOMBRECOMER, NOMPIEZA FROM VENDEDOR, PIEZA, PRECIOSUM WHERE DESCUENTO IS NOT NULL AND DESCUENTO >= 1;

--4. Selecciona la media más alta de descuento de todos los productos que pueden sernos suministrados

SELECT AVG(DESCUENTO) FROM PRECIOSUM GROUP BY NUMVEND HAVING AVG(DESCUENTO) = ( SELECT MAX(AVG(DESCUENTO)) FROM PRECIOSUM GROUP BY NUMVEND );
