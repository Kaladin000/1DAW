...
SELECT v.numvend, v.nomvend FROM vendedor v JOIN pedido p1 ON v.numvend = p1.numvend WHERE numpedido NOT IN (SELECT p2.numpedido FROM pedido p2 WHERE EXTRACT(YEAR from p2.fecha) = 1992);

// 2

SELECT v.nomvend, v.numvend, p.nompieza, MAX(ps.preciounit) FROM vendedor v JOIN preciosum ps ON v.numvend = ps.numvend JOIN pieza p ON ps.numpieza = p.numpieza GROUP BY v.nomvend, v.numvend, p.nompieza, ps.preciounit HAVING ps.preciounit > 50;

// 3

SELECT p.numpedido, p.fecha FROM PEDIDO p WHERE p.numpedido IN (SELECT lp.numpedido FROM linped lp JOIN preciosum ps ON lp.numpieza = ps.numpieza WHERE lp.fecharecep > (p.fecha + ps.diassum) AND cantrecibida < cantpedida);

// 4

SELECT p.nompieza, i.cantdisponible FROM pieza p JOIN inventario i ON p.numpieza = i.numpieza JOIN preciosum ps ON i.numpieza = ps.numpieza JOIN vendedor v ON ps.numvend = v.numvend WHERE i.cantdisponible < i.cantminima AND v.provincia = 'VALENCIA';

// 5

SELECT p.numpieza, p.nompieza, AVG(ps.descuento) FROM PIEZA p JOIN preciosum ps ON p.numpieza = ps.numpieza GROUP BY p.numpieza, p.nompieza, ps.descuento HAVING ps.descuento > 5;

// 6

SELECT p.numpedido, p.fecha FROM pedido p JOIN preciosum ps ON p.numvend = ps.numvend WHERE ps.numpieza IN (SELECT i.numpieza FROM inventario i) GROUP BY p.numpedido, p.fecha HAVING AVG(preciounit) < 100;

// 7

SELECT v.numvend, v.nomvend, (preciocompra*cantpedida) FROM LINPED lp JOIN pedido p ON lp.numpedido = p.numpedido JOIN vendedor v ON p.numvend = v.numvend WHERE EXTRACT(YEAR from p.fecha) = 1992 AND EXTRACT(MONTH from p.fecha) = 12 AND EXTRACT(MONTH from p.fecha) = 11 AND EXTRACT(MONTH from p.fecha) = 10 ORDER BY (preciocompra*cantpedida) DESC;
