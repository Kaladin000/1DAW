...
SELECT NUMVEND,NOMVEND FROM VENDEDOR WHERE NUMVEND IN (SELECT NUMVEND FROM PEDIDO WHERE FECHA >= TO_DATE('01/01/95','DD/MM/YY') AND FECHA <= TO_DATE('31/12/95','DD/MM/YY')) AND NUMVEND NOT IN (SELECT NUMVEND FROM PEDIDO WHERE FECHA >= TO_DATE('01/01/92','DD/MM/YY') AND FECHA <= TO_DATE('31/12/92','DD/MM/YY'));

SELECT v.NUMVEND, v.NOMVEND FROM vendedor v JOIN pedido p ON v.numvend = p.numvend WHERE EXTRACT(YEAR from p.fecha) = 1995 AND p.numvend NOT IN (SELECT numvend FROM pedido p2 WHERE EXTRACT(YEAR from p2.fecha) = 1992)
--OK Puedes utilizar BETWEEN con las fechas y utiliza 4 digitos para definir el año y no tener problemas. También podrías haber utilizado MINUS para facilitar la consulta
...
SELECT NUMPIEZA, NOMPIEZA FROM PIEZA WHERE NUMPIEZA IN (SELECT NUMPIEZA FROM PRECIOSUM WHERE NUMVEND IN (SELECT NUMVEND FROM VENDEDOR WHERE PROVINCIA='ALICANTE') AND NUMPIEZA IN (SELECT NUMPIEZA FROM LINPED WHERE CANTPEDIDA>500));


SELECT pz.NUMPIEZA, pz.NOMPIEZA FROM PIEZA pz WHERE pz.numpieza IN ( SELECT ps.numpieza FROM preciosum ps JOIN vendedor v ON ps.numvend = v.numvend WHERE v.provincia = 'Alicante' GROUP BY ps.numpieza HAVING COUNT(v.numvend) > 2 ) AND pz.numpieza IN ( SELECT lp.numpieza FROM LINPED lp WHERE lp.cantpedida > 500 )
--Tienes que agrupar la ultima SELECT por numpieza y no compruebas que las piezas puedan ser suministras por dos vendedores
/*
select p.numpieza, p.nompieza from preciosum pr join vendedor v on v.numvend=pr.numvend join pieza p on p.numpieza=pr.numpieza
where v.provincia='ALICANTE' and p.numpieza in (select l.numpieza from linped l group by l.numpieza having sum(cantpedida)>500)
group by p.numpieza,p.nompieza having count(v.numvend)>2;
*/
...
SELECT V.NUMVEND, V.NOMVEND, P.NUMPIEZA, P.NOMPIEZA, L.CANTPEDIDA FROM VENDEDOR V, PIEZA P, LINPED L WHERE PROVINCIA='ALICANTE' AND NUMVEND IN (SELECT NUMVEND FROM PEDIDO) AND NUMVEND IN (SELECT NUMVEND FROM PRECIOSUM PS WHERE DIASSUM >= 7);
--Aquí no compruebas que sea la pieza que te han pedido la que pueda ser suministrada en 7 días sino que el vendedor puede suministrarte una pieza en 7 dias o más. Necesitas agrupar para mostrar con el COUNT la cantidad de pedidos distintos en las que te ha sido solicitada
/*
select v.numvend,nomvend,pr.numpieza,nompieza, count(distinct l.numpedido) from preciosum pr join pieza p on pr.numpieza = p.numpieza join vendedor v on v.numvend=pr.numvend join pedido pe on pe.numvend=v.numvend join linped l on l.numpedido=pe.numpedido
where provincia='ALICANTE' and diassum>7 group by v.numvend,nomvend,pr.numpieza,nompieza order by 5; */
...
SELECT * FROM INVENTARIO WHERE NUMPIEZA NOT IN (SELECT NUMPIEZA FROM LINPED);
--Tienes que crear una vista con CREATE VIEW

CREATE VIEW piezas_sin_pedido AS SELECT i.* FROM inventario i WHERE i.numpieza NOT IN ( SELECT DISTINCT lp.numpieza FROM linped lp )
...
SELECT P.NOMPIEZA,PS.NUMPIEZA,PS.PRECIOUNIT,PS.DESCUENTO FROM PRECIOSUM PS, PIEZA P;

--Tienes que utilizar un RIGHT JOIN para que aparezcan todas las piezas y los datos de los suministros si lo tuvieran
